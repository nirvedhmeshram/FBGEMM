...
def main(
    ...
    def fbgemm_hbc_by_feature_preprocess_cpu(input: Tensor) -> Tuple[Tensor, Tensor]:
        return torch.ops.fbgemm.histogram_binning_calibration_by_feature_preprocess(
            input,
            segment_values,
            segment_offsets,
            num_segments,
    ...
    for step in range(iters + warmup_runs):
        for data_type in data_types:
            ...
            if step >= warmup_runs:
                ...
                total_time["hbc_by_feature_preprocess"]["cpu"][data_type] += hbc_by_feature_preprocess_time
        ...
        if torch.cuda.is_available():
            ...
            segment_values_gpu: Tensor = segment_values.cuda()
            segment_lengths_gpu: Tensor = segment_lengths.cuda()
            segment_offsets_gpu: Tensor = segment_offsets.cuda()
            ...
            def fbgemm_hbc_by_feature_preprocess_gpu(input: Tensor) -> Tuple[Tensor, Tensor]:
                return torch.ops.fbgemm.histogram_binning_calibration_by_feature_preprocess(
                    input,
                    segment_values_gpu,
                    segment_offsets_gpu,
            ...
            for data_type in data_types:
                ...
                if step >= warmup_runs:
                    ...
                    total_time["hbc_by_feature_preprocess"]["gpu"][
                        data_type
                    ] += hbc_by_feature_preprocess_time
