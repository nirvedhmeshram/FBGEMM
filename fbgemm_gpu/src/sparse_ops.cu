...
//starting pre processing hbc_feature_code
template <typename T>
__global__ void histogram_binning_calibration_by_feature_preprocess_kernel(
    const int64_t num_logits,
    const int64_t num_bins,
    const int64_t num_segments,
    const double recalibrate_value,
...
std::tuple<Tensor, Tensor> histogram_binning_calibration_by_feature_preprocess_cuda(
    const Tensor& logit,
    const Tensor& segment_value,
    const Tensor& segment_lengths,
    int64_t num_segments,
  ...
  // Convert lengths to offsets for better handling on GPUs.
  const auto segment_lengths_packed = segment_lengths.contiguous();
  auto segment_offsets = segment_lengths_packed.view(-1);
  ...
  const auto bin_num_positives_packed = bin_num_positives.contiguous();
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      logit.type(), "histogram_binning_calibration_by_feature_preprocess_cuda", [&]() {
        histogram_binning_calibration_by_feature_preprocess_kernel<scalar_t>
            <<<fbgemm_gpu::div_round_up(logit.numel(), num_threads),
               num_threads,
               0,
               at::cuda::getCurrentCUDAStream()>>>(
